    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (child.getBottom() < 0) {              //if(child.getTop() < 0) {
                    offsetY = child.getTop();             //    offsetY = - chile.getTop();
                } else if (child.getTop() > maxH) {       //}else if(child.getBottom() > maxH){
                    offsetY = maxH - child.getBottom();   //    offsetY = maxH - child.getBottom();
                }                                         //}
                int w;
                int h;
